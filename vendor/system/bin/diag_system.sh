...
function mergeS()
{
	...
	tail -n 50 $DIAGNOSE_TXT >>  $DIAGNOSE_MINI
...
#we record the last 30 times of crash, each is about 50 lines	
	...
	tail -n 1500 $DIAGNOSE_MINI >  $RESULT_TXT
...
grep -a  -A 50 "$SE_MTK_KEY" $LAST_LOGFILE  > $DIAGNOSE_TXT
...
grep -a -B 5 -A 45 "$SE_WD_KEY" $LAST_LOGFILE  > $DIAGNOSE_TXT
...
grep -a -B 5 -A 45 "$SE_SP_KEY" $LAST_LOGFILE  > $DIAGNOSE_TXT
...
grep -a -A 50	"$SE_KEY" $LAST_LOGFILE  > $DIAGNOSE_TXT
